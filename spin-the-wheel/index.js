...
colors =['#aad401','#44aa00','#2ca089','#00aad4','#3837c8','#5a2ca0','#8800aa','#ff00cc','#ff0066','#d40001','#ff6601','#ffcc01']
...
function showColorGrid (){ 
    ...
    if(!isGridOpen){
        for(let i = 0;i<colors.length;i++){
            pickColrGrid +=  `<button class="colrPicked" id="${colors[i]}" style='background-color:${colors[i]}' onclick="pickColorFunc('${colors[i]}')">me</button>`;
        }
        document.querySelector('.pickColrGrid').innerHTML = pickColrGrid;
        isGridOpen = true
    }
    else{
        document.querySelector('.pickColrGrid').innerHTML = '';
        isGridOpen = false
    }
}
...
const startButton = document.querySelector('.buttonRoll');
...
function alertResult(actualDeg) {
    ...
    if(result === choice){
        ...
        colorResult.style.fontWeight = 'bold';
        colorResult.style.fontSize = '30px';
        colorResult.innerHTML = 'You Loss'
        colorResult.style.position = 'absolute';
        colorResult.style.transform = 'translateX(-50%)';
        colorResult.style.left = '50%';
        colorResult.style.marginTop = '46.5%';
